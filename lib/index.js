(function (global, factory) {
  ...
  class heightmap {
    ...
    renderTiles(tiles) {
      ...
      return _asyncToGenerator(function* () {
        var url = _arguments2.length > 1 && _arguments2[1] !== undefined ? _arguments2[1] : 'https://s3.amazonaws.com/elevation-tiles-prod/normal/{z}/{x}/{y}.png';
        ...
        for (var y in tiles) {
          ...
          for (var x in tiles[y]) {
            var tmp_url = url.replace('{z}', tiles[y][x].z).replace('{x}', tiles[y][x].x).replace('{y}', tiles[y][x].y);
            parallel_x_buf.push(axios({
              url: tmp_url,
          ...
          var callthem = yield Promise.all(parallel_x_buf);

          for (var _x2 in callthem) {
            renderObj.push({
              src: callthem[_x2].data,
              x: tiles[y][_x2].pos_x * 256,
              y: tiles[y][_x2].pos_y * 256
            });
        ...
        var map64 = yield mergeImages(renderObj, {
          Canvas,
          Image
        });
        yield fs.writeFile('test.png', Buffer(map64, 'base64'));
    ...
    getMeshgridWithinPolygon(polygonArray) {
      ...
      return _asyncToGenerator(function* () {
        var distance = _arguments3.length > 1 && _arguments3[1] !== undefined ? _arguments3[1] : 30;
        ...
        */
        ...
        var bounds = geolib.getBounds(); //step2: get steps value between points

        var width_mt = geolib.getDistance({
        ...
        var steps = Math.ceil(width_mt / distance); //amount of steps
        ...
        for (var y in range) {
          var row = [];
          var row_position = geolib.computeDestinationPoint({
            latitude: bounds.minLat,
            longitude: bounds.minLng
          }, distance * y, 180);

          for (var x in range) {
            var tmp = geolib.computeDestinationPoint(row_position, distance * x, 90);

            if (geolib.isPointInPolygon(tmp, polygonArray)) {
              //step3
              row.push(tmp);
            }
          }
    ...
    runTest() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var tiles = yield _this4.getTiles(mesh);
        yield _this4.renderTiles(tiles); //console.log('row tiles with url',tiles_test);
        //end
