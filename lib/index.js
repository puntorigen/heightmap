  ...
  class heightmap {
    ...
    getMeshgridWithinPolygon(polygonArray) {
      ...
      */
      ...
      var bounds = geolib.getBounds(polygonArray);
      console.log('bounds', bounds); //step2: get steps value between points
      ...
      var points_per_row = Math.ceil(width_mt / distance); //amount of steps

      console.log('steps in width', points_per_row); //step2: create grid points separated within the specified distance
      //step3: at same time, check which points are within the original polygon

      var grid = []; //let pointInPolygon = require('point-in-polygon');

      var range = Array(points_per_row).fill(0);
      ...
      for (var y in range) {
        ...
        for (var x in range) {
          var tmp = geolib.computeDestinationPoint(row_position, distance * x, 90); //let test = geolib.isPointInPolygon(tmp, polygonArray);
          //console.log('testing point within polygon',{ row:row_position, test,tmp });

          row.push(tmp); //if (test) { //step3
